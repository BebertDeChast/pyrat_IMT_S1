...
def give_score(graph, current_vertex, neighbors, opponent_location, player_location):
    ...
    for i in range(len(neighbors)):  # We cycle through our neighbors
        ...
        hq.heappush(scored_neighbors, (distance_table[neighbor] + enemy_proximity_score(graph, neighbor, opponent_location, player_location), neighbor, routing_table))  # For each vertex, we associate to it the distance found
    return scored_neighbors


def enemy_proximity_score(graph, target, enemy_location, player_location):
    """
    Give a score depending of enemy closeness to target. The closer the higher the score is
    Variables :
        target : tuple(int, int), target location
...
def preprocessing(maze_map, maze_width, maze_height, player_location, opponent_location, pieces_of_cheese, time_allowed):
    global target
    trash_route, closest_cheeses = greedy(maze_map, player_location, pieces_of_cheese, opponent_location, player_location, 10)
    target = [closest_cheeses[-1]]
    meta_graph, distance_table = build_meta_graph(maze_map, closest_cheeses, player_location)
    result_best = tsp(meta_graph, player_location)
    route = meta_graph_route_to_route(result_best, distance_table)
    moves_from_locations(route)
...
def turn(maze_map, maze_width, maze_height, player_location, opponent_location, player_score, opponent_score, pieces_of_cheese, time_allowed):
    ...
    # check if opponent is in front of us in path during n turns
    # n_proximity = dijkstra(player_location,maze_map,opponent_location)[1][opponent_location]

    # if n_proximity == proximity:
    #     ahead_of_us += 1
    # else:
    #     ahead_of_us = 0
    # proximity = n_proximity

    # if ahead_of_us == 3:
    #     recalculate = True
    #     print("Enemy ahead of us ! Recalculating !")
